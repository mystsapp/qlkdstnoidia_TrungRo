...
{
    public class ThanhLyController : BaseController
    {
        qlkdtrEntities db = new qlkdtrEntities();
        public ActionResult ThanhLyIndex(decimal id)
        {
              qlkdtrEntities db = new qlkdtrEntities();
            if (Session["username"] == null) return RedirectToAction("login", "Login");
            tourDAO dao = new tourDAO();
            tour model = dao.Details(id);
            string sExt = Path.GetExtension(model.noidungthanhlyhd);
            ViewBag.fileext = sExt;
            ViewBag.UploadPath = System.Configuration.ConfigurationManager.AppSettings["UploadPath"].ToString();
            ViewBag.ViewUploadPath = System.Configuration.ConfigurationManager.AppSettings["ViewUploadPath"].ToString();
